...
void displayPatientData(const struct Patient* patient, int fmt)
{
    ...
    {
        printf("%05d %-15s ", patient->number_of_patients, patient->name);
        displayFormattedPhone(patient->phone.number);
        printf(" (%s)\n", patient->phone.description);
    }
}
...
void menuPatientEdit(struct Patient* patient)
{
    ...
    do
    {
        printf("Edit Patient (%05d)\n"
            "=========================\n"
            "1) NAME : %s\n"
            "2) PHONE: ", patient->number_of_patients, patient->name);
...
void addPatient(struct Patient patient[], int max)
{
    ...
    {
        patient[free_index].number_of_patients = nextPatientNumber(patient, max);
...
void removePatient(struct Patient patient[], int max)
{
    ...
    else
    {
        ...
        printf("Name  : %s\n", patient[index].name);
        printf("Number: %05d\n", patient[index].number_of_patients);
        printf("Phone : ");
...
void searchPatientByPhoneNumber(const struct Patient patient[], int max)
{
    ...
    {
        // display a single eligible patient record
        if (strcmp(phone_number, patient[counter].phone.number) == 0)
        {
            flag = 1;
            break;
        }
...
int nextPatientNumber(const struct Patient patient[], int max)
{
    ...
    for (counter = 0; counter < max; counter++)
    {
        if (patient[counter].number_of_patients > next_highest_patient)
        {
            next_highest_patient = patient[counter].number_of_patients;
        }
    }
...
int findPatientIndexByPatientNum(int patientNumber, const struct Patient patient[], int max)
{
    ...
    for (counter = 0; counter < max; counter++)
    {
        ...
        if (patient[counter].number_of_patients == patientNumber)
        {
            flag2 = counter;
            break;
        }
